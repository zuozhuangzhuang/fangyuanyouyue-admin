...
(function (window, document, $) {
    ...
    $.site.contentTabs = {
        ...
        closeTab: function ($item) {
            var $current = $item || $("#admui-siteConTabs").find("ul.con-tabs").find("li.active"),
                tabId = $current.children('a').attr('target'), $checked = '',
                $nextTab = $current.next('li'),
                $content = this.$content;

            if ($current.is('.active')) { // 关闭选中标签
                if ($nextTab.size() > 0) {
                    $checked = $nextTab;
                } else {
